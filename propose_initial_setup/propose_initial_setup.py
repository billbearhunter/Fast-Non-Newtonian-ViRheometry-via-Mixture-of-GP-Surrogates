...
def main():
    ...
    print(f"  -  config_00.png      (calibration reference frame)")
    print(f"  -  config_01..08.png  (8 measurement frames)")
    print()
    print(f"Settings: material={args.material_name}, H={Hcm} cm, W={Wcm} cm, density={args.density}")
    print()
    print(f"Then process the photos and run setup1 (production config):")
    print(f"  # 1. Extract per-frame flow distance from photos")
    print(f"  python Calibration/extract_flow_distance.py --dir {out_dir}")
    print()
    print(f"  # 2. Convert flow_distances.csv -> y_obs.npy")
    print(f"  python -m Calibration.build_y_obs --dir {out_dir}")
    print()
    print(f"  # 3. setup1 (single-setup inverse + Hessian-orthogonal proposal of setup2)")
    print(f"  python -m Optimization.setup1 \\")
    print(f"      --state-root Models/v10_yshape_round2_sy \\")
    print(f"      -f {out_dir} --density {args.density} \\")
    print(f"      --shape-l2-mode eps --shape-l2-eps-cm 0.5 --shape-eps-cap-by-primary \\")
    print(f"      --shape-top-k 10 --shape-weight-mode gp_fit \\")
    print(f"      --shape-gpfit-oversample 1 \\")
    print(f"      --shape-popsize 8 --shape-max-iter 20")
    print()
    print(f"After taking the second experiment at the proposed (W*, H*), run joint setup2:")
    print(f"  python -m Optimization.setup2 \\")
    print(f"      --state-root Models/v10_yshape_round2_sy \\")
    print(f"      -f {out_dir} \\")
    print(f"      -s data/.../ref_{args.material_name}_<H*>_<W*>_2 \\")
    print(f"      --shape-l2-mode eps --shape-l2-eps-cm 0.5 --shape-eps-cap-by-primary \\")
    print(f"      --shape-top-k 2 --shape-weight-mode gp_fit \\")
    print(f"      --shape-warm-start-from-first \\")
    print(f"      --shape-sy-prior-weight 2.0 \\")
    print(f"      --shape-popsize 8 --shape-max-iter 20")
