...
def _read_rheo(file):
    # Different export templates put the units row ([s], [1/s], [Pa], ...) at
    # different positions, so auto-detect the header row.
    for h in range(0, 12):
        try:
            ...
            continue
    raise RuntimeError(f"Could not locate header row with '[1/s]' and '[Pa]' in {file}")
