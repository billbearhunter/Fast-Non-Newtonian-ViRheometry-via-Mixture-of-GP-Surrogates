...
def partition_one_gid(df_g: pd.DataFrame, gid: int,
    ...
    `sy_weight > 0` augments the KMeans feature vector with a normalised
    log(sigma_y + 1) column, so clusters are sigma_y-aware.  Routing
    centroids (centroid_pc) are always stored in the shape-only PCA space
    so that inference-time routing (which cannot observe sigma_y) is
    unaffected.
    """
    ...
    for b in range(len(edges) - 1):
        ...
        # Build KMeans feature matrix: PCA shape + optional log(sigma_y+1)
        if sy_weight > 0.0:
            log_sy = np.log(X_phys[idx, 2] + 1.0)  # sigma_y=0 -> 0; safe
            sy_std = log_sy.std() + 1e-8
            log_sy_sc = (log_sy - log_sy.mean()) / sy_std * sy_weight
            pcs_km = np.column_stack([pcs, log_sy_sc])
        else:
            pcs_km = pcs

        if k_sh_eff == 1:
            labels = np.zeros(len(idx), dtype=int)
            centers = pcs.mean(axis=0, keepdims=True)  # shape-only centroid
        else:
            km = KMeans(n_clusters=k_sh_eff, n_init=10, random_state=rng_seed)
            labels = km.fit_predict(pcs_km)
            # Merge tiny clusters (use augmented space for distance, same as KMeans)
            centers_km = km.cluster_centers_.copy()
            for _ in range(k_sh_eff):
                counts = np.array([(labels == s).sum() for s in range(len(centers_km))])
                tiny = np.where((counts > 0) & (counts < min_n_per_sub // 2))[0]
                if len(tiny) == 0:
                    break
                t = tiny[0]
                # find nearest non-tiny centroid
                d = np.linalg.norm(centers_km - centers_km[t][None, :], axis=1)
                d[t] = np.inf
                d[counts == 0] = np.inf
                d[(counts > 0) & (counts < min_n_per_sub // 2) & (np.arange(len(centers_km)) != t)] = np.inf
            ...
            k_sh_eff = len(unique)
            # Routing centroids: always shape-only PCA space (inference has no sigma_y)
...
def split_subs_by_sigma_y(
    state: dict,
    df_g: pd.DataFrame,
    min_n: int = 600,
    span_thr: float = 2.0,
) -> tuple:
    """Post-process: binary-split subs with wide sigma_y span at median log(sigma_y+1).

    For each sub with N >= min_n and log(sigma_y+1) range > span_thr:
      - lo half: sigma_y <= median  → keeps original sub_id
      - hi half: sigma_y >  median  → gets a new sub_id (max+1)
    Routing centroid_pc is recomputed in shape PCA space for each half.
    mu_z / cov_z are updated to reflect the narrowed parameter range.

    Returns (new_state, updated_sub_assignments).
    """
    import copy

    cols_Y = [f"x_{i:02d}" for i in range(1, 9)]
    cols_X = ["n", "eta", "sigma_y", "width", "height"]
    X_phys = df_g[cols_X].to_numpy(np.float64)
    Y_all  = df_g[cols_Y].to_numpy(np.float64)

    all_ids = [sub["sub_id"] for bin_d in state["bins"] for sub in bin_d["subs"]]
    next_id = (max(all_ids) + 1) if all_ids else 0

    # Rebuild sub_assignments array from member_idx
    N = len(df_g)
    sub_asgn = np.full(N, -1, dtype=int)
    for bin_d in state["bins"]:
        for sub in bin_d["subs"]:
            sub_asgn[sub["member_idx"]] = sub["sub_id"]

    new_state = copy.deepcopy(state)
    split_count = 0

    for bin_d in new_state["bins"]:
        pca_d = bin_d["pca"]
        scl_d = bin_d["scaler"]
        new_subs = []

        for sub in bin_d["subs"]:
            members = sub["member_idx"]
            N_sub   = len(members)
            ...
            sy     = X_phys[members, 2]
            log_sy = np.log(sy + 1.0)
            span   = float(log_sy.max() - log_sy.min())

            if span <= span_thr:
                new_subs.append(sub)
                continue

            # Split at median log(sigma_y+1)
            thr        = float(np.median(log_sy))
            lo_mask    = log_sy <= thr
            hi_mask    = ~lo_mask
            lo_members = members[lo_mask]
            hi_members = members[hi_mask]

            if len(lo_members) < 20 or len(hi_members) < 20:
                new_subs.append(sub)
                continue

            def _centroid(idx, _pca=pca_d, _scl=scl_d):
                nc   = norm_curve(Y_all[idx])
                nc_s = (nc - _scl["mean"]) / _scl["std"]
                pcs  = (nc_s - _pca["mean"]) @ _pca["components"].T
                return pcs.mean(axis=0)

            def _mu_cov(idx):
                Xl  = X_phys[idx]
                z   = np.column_stack([
                    Xl[:, 0],
                    np.log(np.clip(Xl[:, 1], 1e-12, None)),
                    np.log(np.clip(Xl[:, 2], 1e-12, None)),
                ])
                mu  = z.mean(axis=0)
                cov = np.cov(z.T) if len(z) > 1 else np.diag([0.04, 0.36, 0.36])
                return mu, cov

            # Update lo half (keeps original sub_id)
            mu_lo, cov_lo      = _mu_cov(lo_members)
            sub["N"]           = int(len(lo_members))
            sub["centroid_pc"] = _centroid(lo_members)
            sub["mu_z"]        = mu_lo
            sub["cov_z"]       = cov_lo
            sub["member_idx"]  = lo_members
            new_subs.append(sub)

            # Create hi half (new sub_id)
            mu_hi, cov_hi = _mu_cov(hi_members)
            hi_sub = {
                "sub_id":      int(next_id),
                "bin_id":      sub["bin_id"],
                "shape_id":    sub["shape_id"],
                "centroid_pc": _centroid(hi_members),
                "N":           int(len(hi_members)),
                "mu_z":        mu_hi,
                "cov_z":       cov_hi,
                "member_idx":  hi_members,
            }
            new_subs.append(hi_sub)
            sub_asgn[hi_members] = next_id

            print(f"    sy-split sub {sub['sub_id']} "
                  f"(N={N_sub}, span={span:.1f}): "
                  f"lo={len(lo_members)} hi={len(hi_members)} → new id={next_id}")
            next_id    += 1
            split_count += 1

        bin_d["subs"] = new_subs

    new_state["n_subs_total"] = next_id
    if split_count:
        print(f"  sigma_y split: {split_count} subs → total {next_id} subs")
    return new_state, sub_asgn
...
def main():
    ...
    ap.add_argument("--sy-weight", type=float, default=0.0,
                    help="Weight for log(sigma_y+1) in KMeans feature vector "
                         "(0 = disabled, 1.5 = recommended). Routing centroids "
                         "stay in shape-only PCA space so inference is unaffected.")
    ...
    ap.add_argument("--sy-split-min-n", type=int, default=0,
                    help="Sigma_y split: minimum sub N to trigger binary split at median "
                         "log(sigma_y+1). 0 = disabled. Recommended: 600.")
    ap.add_argument("--sy-split-span-thr", type=float, default=2.0,
                    help="Sigma_y split: minimum log(sigma_y+1) span to trigger split "
                         "(default 2.0). Only active when --sy-split-min-n > 0.")
    args = ap.parse_args()
    ...
    if args.infill_glob:
        ...
        infill_paths = sorted(Path(p) for p in _glob.glob(args.infill_glob))
        if infill_paths:
    ...
    for gid, df_g in df.groupby("gid"):
        ...
        state, assignments = partition_one_gid(
            ...
            sy_weight=args.sy_weight,
        )
        if args.sy_split_min_n > 0:
            print(f"  applying sigma_y split "
                  f"(min_n={args.sy_split_min_n}, span>{args.sy_split_span_thr})...")
            state, assignments = split_subs_by_sigma_y(
                state, df_g,
                min_n=args.sy_split_min_n,
                span_thr=args.sy_split_span_thr,
            )
