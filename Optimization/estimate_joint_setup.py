...
Pipeline:
    ...
    2. Joint inverse: y8-quantile routing + GP-aware likelihood over both setups.
    3. Write `<second_dir>/theta_hat.json` with the joint theta_hat.
...
from Optimization.estimate_first_setup import parse_ref_dir


def estimate_joint_setup(first_dir: Path, second_dir: Path, args) -> dict:
    ...
    item_a = SHA.prepare_setup_for_inverse(first_dir.name, info_a["W"], info_a["H"], y_a,
                                           geo_router, xs, ys, device, dtype, args)
    item_b = SHA.prepare_setup_for_inverse(second_dir.name, info_b["W"], info_b["H"], y_b,
                                           geo_router, xs, ys, device, dtype, args)
    res = SHA.inverse_joint_setup(item_a, item_b, args, device, dtype)
    if res is None:
...
def main():
    ...
    out = estimate_joint_setup(args.first_dir.resolve(), args.second_dir.resolve(), args)
    (args.second_dir / "theta_hat.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
