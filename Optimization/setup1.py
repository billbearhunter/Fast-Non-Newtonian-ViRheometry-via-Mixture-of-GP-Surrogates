...
Pipeline:
    ...
    2. Inverse: CMA-ES + σY two-stage prior over the surrogate (Plan B).
    ...
    5. Write `<ref_dir>/theta_hat.json` with θ̂ + proposed setup info.
...
def estimate(ref_dir: Path, args) -> dict:
...
def main():
    ...
    print(
        ...
        f"\n\nBefore running setup2.py, place into the folder above:"
        f"\n  - y_obs.npy  (8-frame flow distance from 2nd experiment)"
        f"\n  - camera_params.xml  (calibration)"
        f"\n\nThen run:"
        f"\n  python -m Optimization.setup2 -f {args.first_dir} -s {out['proposed_setup_2']['dir']}"
    )
