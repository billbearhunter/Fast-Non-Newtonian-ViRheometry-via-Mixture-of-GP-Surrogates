...
Public entry points:
   prepare_setup_for_inverse(setup_name, W, H, y_obs, geo_router, xs, ys, device, dtype, args)
   inverse_single_setup(item, args, device, dtype)
   inverse_joint_setup(item_a, item_b, args, device, dtype)
...
ROUTING_PROFILES = {
    # Real-video deployment: robust terminal-displacement evidence under
    # sim-real/camera mismatch.  This is the Chuno/Okonomiyaki production path.
    "real_robust": {
        "weight_mode": "y8_quantile",
        "top_k": 1,
        "oversample": 3,
        "noise_floor_cm": 0.0,
    },
    # Simulator-matched validation: all eight frames are generated by the same
    # simulator as the surrogate, so full-trajectory evidence is trusted.
    # The final inverse remains top-1 to keep the CMA box local and narrow.
    "sim_precision": {
        "weight_mode": "full_nn",
        "top_k": 1,
        "oversample": 10,
    ...
    # Joint setup-2 consistency rerank: among the PCA-nearest observation
    # candidates for setup-2's (W2, H2, y_obs2), pick the sub whose
    # parameter cloud (z_nn) lies closest to setup-1's posterior estimate
    # θ̂_1.  Rationale: if setup-1 already places the truth near θ̂_1, the
    # second-leg sub should at least *cover* that θ neighbourhood; otherwise
    # the joint loss mixes two GP basins that disagree on θ.  Requires
    # args.shape_z_anchor (a 3-vector in z-space (n, log η, log σ_Y));
    # falls back to full_nn when the anchor is not provided.
    "joint_consistency": {
        "weight_mode": "joint_consistency",
        "top_k": 1,
        "oversample": 10,
        "noise_floor_cm": 0.0,
    },
...
def _resolve_routing_config(args) -> dict:
    """Resolve the observation-evidence routing profile into concrete knobs.

    Existing scripts may still pass shape_weight_mode directly; that explicit
    value wins and preserves backward compatibility.  If shape_weight_mode is
    omitted/None, shape_routing_profile selects a named profile.
    """
    profile = getattr(args, "shape_routing_profile", None)
    explicit_mode = getattr(args, "shape_weight_mode", None)
    if explicit_mode is None:
        profile = profile or "real_robust"
        if profile not in ROUTING_PROFILES:
            raise ValueError(f"unknown --shape-routing-profile={profile}")
        defaults = ROUTING_PROFILES[profile]
        return {
            "profile": profile,
            "weight_mode": defaults["weight_mode"],
            "top_k": int(defaults["top_k"]),
            "oversample": int(defaults["oversample"]),
            "noise_floor_cm": float(defaults["noise_floor_cm"]),
        }
    return {
        "profile": profile or "custom",
        "weight_mode": str(explicit_mode),
        "top_k": int(getattr(args, "shape_top_k", 1)),
        "oversample": int(getattr(args, "shape_gpfit_oversample", 3)),
        "noise_floor_cm": float(getattr(args, "shape_noise_floor_cm", 0.0)),
    }
...
class SubMember:
    ...
    # Used by y8_quantile re-rank to score |y_obs[7] - y8_med| / y8_std.
    y8_med: float = 0.0
    y8_std: float = 1.0
    # Per-frame median + std of training Y for "unified" weight_mode (8-D
    # generalization of y8_med/y8_std).  Defaults are no-ops for older
    # checkpoints; populated by _build_sub_member when training data is
    # available.
    y_med_full: np.ndarray = field(default_factory=lambda: np.zeros(8))
    y_std_full: np.ndarray = field(default_factory=lambda: np.ones(8))
    y_train_full: np.ndarray = field(default_factory=lambda: np.empty((0, 8)))
...
def _build_sub_member(sd: Path, sub_id: int, bin_id: int, y_obs: np.ndarray,
    ...
    # Y_8 quantile statistics for sub-level routing (y8_quantile mode).
    ...
    # Per-frame stats for "unified" weight_mode (8-D y8q generalization).
    if Y_phys.shape[0] >= 2 and Y_phys.shape[1] >= 8:
        y_med_full = np.median(Y_phys[:, :8], axis=0).astype(np.float64)
        y_std_full = np.std(Y_phys[:, :8], axis=0, ddof=1).astype(np.float64)
        y_std_full = np.maximum(y_std_full, 0.05)  # floor: 0.5 mm
    else:
        y_med_full = np.zeros(8, dtype=np.float64)
        y_std_full = np.ones(8, dtype=np.float64)
    ...
    return SubMember(
        ...
        y_med_full=y_med_full, y_std_full=y_std_full,
        y_train_full=Y_phys[:, :8].astype(np.float64) if Y_phys.ndim == 2 else np.empty((0, 8)),
    )
...
def _setup_loss_gp_aware_np(y_hat, sigma_pred, y_obs, frame_w,
...
def _run_cma(loss_fn, x0, lo, hi, sigma0=0.25, max_iter=30, popsize=12, seed=7,
              trail_writer=None, trail_restart_idx=0, trail_transform=None):
    """CMA-ES driver.
    ...
    """
    opts = {"popsize": popsize, "verbose": -9,
    ...
    while not es.stop():
        ...
        if trail_writer is not None:
            trail_arr = trail_transform(xs_arr) if trail_transform is not None else xs_arr
            trail_writer(trail_arr, losses_arr, trail_restart_idx, gen_idx)
...
def _run_cma_multistart(loss_fn, x0, lo, hi, sigma0=0.25, max_iter=30, popsize=12,
    ...
    """
    if n_restarts <= 1:
        x, l, n_ev = _run_cma(loss_fn, x0, lo, hi, sigma0, max_iter, popsize, base_seed,
                               trail_writer=trail_writer, trail_restart_idx=0,
                               trail_transform=trail_transform)
        z = trail_transform(x.reshape(1, -1))[0] if trail_transform is not None else x
    ...
    x_best, l_best = results[0]
    x_arr = np.stack([r[0] for r in results])           # (n_restarts, 3)
    z_arr = trail_transform(x_arr) if trail_transform is not None else x_arr
    z_best = trail_transform(x_best.reshape(1, -1))[0] if trail_transform is not None else x_best
    z_dispersion = z_arr.std(axis=0)
    return z_best, l_best, total_evals, z_dispersion


def _z_to_unit_logit(z, lo, hi, eps=1e-6):
    z = np.asarray(z, dtype=np.float64)
    p = (z - lo) / np.maximum(hi - lo, 1e-12)
    p = np.clip(p, eps, 1.0 - eps)
    return np.log(p / (1.0 - p))


def _unit_logit_to_z(u, lo, hi):
    u = np.asarray(u, dtype=np.float64)
    p = 1.0 / (1.0 + np.exp(-np.clip(u, -60.0, 60.0)))
    return lo + (hi - lo) * p


def _prepare_cma_space(loss, x0, lo, hi, args):
    space = str(getattr(args, "shape_cma_space", "z"))
    if space == "z":
        return loss, x0, lo, hi, float(getattr(args, "shape_sigma0", 0.25)), None
    if space != "unit_logit":
        raise ValueError(f"unknown shape_cma_space={space}")
    u0 = _z_to_unit_logit(x0, lo, hi)

    def loss_u(u_or_us):
        return loss(_unit_logit_to_z(u_or_us, lo, hi))

    def trail_transform(u_or_us):
        return _unit_logit_to_z(u_or_us, lo, hi)

    return loss_u, u0, None, None, float(getattr(args, "shape_unit_sigma0", 1.0)), trail_transform


def _make_trail_writer(csv_path, inverse_id: str, fb_lo, fb_hi,
                        W_a: float, H_a: float,
                        W_b: float | None = None, H_b: float | None = None):
    ...
    Each row has the candidate θ + the geometry the inverse used.  For
    single inverses W_b/H_b are blank; for joint inverses they hold the
    second setup's geometry so the offline γ̇ batch can dump γ̇ at both
    setups without extra CLI args.
    """
    import csv
    ...
    Wa_s = f"{float(W_a):.6g}"
    Ha_s = f"{float(H_a):.6g}"
    Wb_s = f"{float(W_b):.6g}" if W_b is not None else ""
    Hb_s = f"{float(H_b):.6g}" if H_b is not None else ""
...
def prepare_setup_for_inverse(setup_name, W, H, y_obs, geo_router, xs, ys,
    ...
    with open(sd / "state.pkl", "rb") as f:
        state = pickle.load(f)
    routing_cfg = _resolve_routing_config(args)
    top_k = max(1, int(routing_cfg["top_k"]))
    l2_mode = str(getattr(args, "shape_l2_mode", "hard"))
    eps_cm = float(getattr(args, "shape_l2_eps_cm", 0.5))
    weight_mode = str(routing_cfg["weight_mode"])
    noise_floor_cm = float(routing_cfg["noise_floor_cm"])
    ...
    over_factor = int(routing_cfg["oversample"])
    fetch_k = top_k * over_factor if weight_mode in ("gp_fit", "y8_quantile", "chi2", "unified", "full_nn", "joint_consistency") else top_k
    ...
    if weight_mode == "y8_quantile":
        ...
        # Historical A/B comparison against y8_quantile.
        fit_dists = []
        for m in over_members:
        ...
        members = [over_members[i] for i in keep]
        d_kept = np.asarray([fit_dists[i] for i in keep], dtype=np.float64)
    elif weight_mode == "full_nn":
        # Simulator-matched retrieval: choose the sub whose training output
        # cloud has the nearest full trajectory to y_obs.  This uses the full
        # 8-frame observation but does not run a GP forward during routing.
        y_obs_v = np.asarray(y_obs, dtype=np.float64)
        nn_dists = []
        for m in over_members:
            y_train = np.asarray(m.y_train_full, dtype=np.float64)
            if y_train.size == 0:
                nn_dists.append(float("inf"))
                continue
            residual = (y_train - y_obs_v[None, :]) * FRAME_W_NP[None, :]
            nn_dists.append(float(np.min(np.sqrt(np.mean(residual * residual, axis=1)))))
        order = np.argsort(np.asarray(nn_dists))
        keep = order[:max(1, top_k)]
        members = [over_members[i] for i in keep]
        d_kept = np.asarray([nn_dists[i] for i in keep], dtype=np.float64)
    elif weight_mode == "joint_consistency":
        # Joint setup-2 consistency rerank.  Inputs:
        #   - PCA-shortlisted observation candidates (over_members).
        #   - args.shape_z_anchor = z(θ̂_1), a 3-vector in z-space
        #     (n, log η, log σ_Y) coming from the setup-1 single-setup
        #     posterior.
        # Score: ‖m.z_nn - z_anchor‖_Z  with Z_SCALE per-axis normalisation.
        # When the anchor is missing we fall back to full_nn.
        z_anchor = getattr(args, "shape_z_anchor", None)
        y_obs_v = np.asarray(y_obs, dtype=np.float64)
        if z_anchor is None:
            # Fallback: full_nn behaviour.  Prevents silent mis-routing if a
            # caller forgets to set the anchor.
            nn_dists = []
            for m in over_members:
                y_train = np.asarray(m.y_train_full, dtype=np.float64)
                if y_train.size == 0:
            ...
            anchor = np.asarray(z_anchor, dtype=np.float64).reshape(3)
            z_dists = []
            for m in over_members:
                z_diff = (m.z_nn - anchor) / Z_SCALE_NP
                z_dists.append(float(np.sqrt(np.sum(z_diff * z_diff))))
            order = np.argsort(np.asarray(z_dists))
            keep = order[:max(1, top_k)]
            d_kept = np.asarray([z_dists[i] for i in keep], dtype=np.float64)
        members = [over_members[i] for i in keep]
    elif weight_mode == "unified":
        # Unified score: per-frame Mahalanobis with three-source variance.
        # Generalizes y8_quantile (which uses only d=7 with sub training std)
        # to all 8 frames + GP variance + measurement noise floor.
        #   sigma_eff_d^2 = sigma_GP_d^2 + sigma_train_d^2 + sigma_noise^2
        # Sim: sigma_noise=0  -> sigma_train dominates, behaves like 8-D y8q
        # Real: sigma_noise>0 -> noise inflates denom, robust to extraction err
        nf2 = noise_floor_cm ** 2
        unified_dists = []
        y_obs_v = np.asarray(y_obs, dtype=np.float64)
        for m in over_members:
            y_pred, sigma_pred = _predict_one(m.z_nn, m, ctx)
            residual = np.asarray(y_pred, dtype=np.float64) - y_obs_v
            sig_gp2 = np.maximum(np.asarray(sigma_pred, dtype=np.float64) ** 2, 1e-8)
            sig_train2 = np.asarray(m.y_std_full, dtype=np.float64) ** 2
            sigma2_eff = sig_gp2 + sig_train2 + nf2
            score = float(np.sum(residual ** 2 / sigma2_eff))
            unified_dists.append(score)
        order = np.argsort(np.asarray(unified_dists))
        keep = order[:max(1, top_k)]
        members = [over_members[i] for i in keep]
        d_kept = np.asarray([unified_dists[i] for i in keep], dtype=np.float64)
    elif weight_mode == "chi2":
        # Noise-aware GP-fit: residual under (sigma_GP^2 + sigma_noise^2).
        # Unifies y8_quantile (real-data robustness) and gp_fit (sim sharpness)
        # via a single noise_floor_cm that's set at deployment time:
        #   noise_floor_cm = 0   -> synthetic mode, sharpest selection
        #   noise_floor_cm > 0   -> real-camera mode, robust to extraction noise
        nf2 = noise_floor_cm ** 2
        chi2_dists = []
        y_obs_v = np.asarray(y_obs, dtype=np.float64)
        for m in over_members:
            y_pred, sigma_pred = _predict_one(m.z_nn, m, ctx)
            residual = y_pred - y_obs_v
            sigma2_eff = np.maximum(np.asarray(sigma_pred, dtype=np.float64) ** 2, 1e-8) + nf2
            chi2 = float(np.sum(residual ** 2 / sigma2_eff))
            chi2_dists.append(chi2)
        order = np.argsort(np.asarray(chi2_dists))
        keep = order[:max(1, top_k)]
        members = [over_members[i] for i in keep]
        d_kept = np.asarray([chi2_dists[i] for i in keep], dtype=np.float64)
    ...
    return {
        ...
        "routing_profile": routing_cfg["profile"],
        "routing_weight_mode": weight_mode,
        "routing_oversample": over_factor,
        "routing_noise_floor_cm": noise_floor_cm,
    }
...
def inverse_single_setup(item, args, device, dtype) -> dict | None:
    if not item["members"]:
        return None
    ...
    def loss(z_or_zs):
        ...
        for i, m in enumerate(members):
            ...
            for p in range(P):
                losses_mat[i, p] = _setup_loss_gp_aware_np(
    ...
    if trail_csv:
        trail_writer = _make_trail_writer(
            ...
            fb_lo=lo, fb_hi=hi,
            W_a=item["W"], H_a=item["H"],
        )
    loss_opt, x0_opt, lo_opt, hi_opt, sigma0_opt, trail_transform = _prepare_cma_space(
        loss, x0, lo, hi, args
    )
    z_best, l_best, n_ev, z_disp = _run_cma_multistart(
        loss_opt, x0_opt, lo_opt, hi_opt,
        sigma0=sigma0_opt,
...
def inverse_joint_setup(item_a, item_b, args, device, dtype) -> dict | None:
    if not item_a["members"] or not item_b["members"]:
        return None
    pair_mode = str(getattr(args, "shape_joint_pair_mode", "mixture"))
    if pair_mode == "per_pair" and (len(item_a["members"]) > 1 or len(item_b["members"]) > 1):
        return _inverse_joint_setup_per_pair(item_a, item_b, args, device, dtype)
    ...
    # estimate_joint_setup.py reads <first_dir>/theta_hat.json and writes the unpacked z
    ...
    if trail_csv:
        trail_writer = _make_trail_writer(
            ...
            fb_lo=lo, fb_hi=hi,
            W_a=item_a["W"], H_a=item_a["H"],
            W_b=item_b["W"], H_b=item_b["H"],
        )
    loss_opt, x0_opt, lo_opt, hi_opt, sigma0_opt, trail_transform = _prepare_cma_space(
        loss, x0, lo, hi, args
    )
    ...
    raw_wrms_a = relative_wrms(y_a, item_a["y_obs"])
    raw_wrms_b = relative_wrms(y_b, item_b["y_obs"])
    raw_wrms = raw_wrms_a + raw_wrms_b
    ...
    out = {
        ...
        "objective": l_best,
        "raw_wrms_total": raw_wrms,
        "raw_wrms_a": raw_wrms_a,
        "raw_wrms_b": raw_wrms_b,
        "flow_total": 0.0,
...
def _with_single_member(item: dict, member: SubMember) -> dict:
    out = dict(item)
    out["members"] = [member]
    out["member_weights"] = [1.0]
    out["bin_id"] = int(member.bin_id)
    out["sub_id"] = int(member.sub_id)
    return out


def _inverse_joint_setup_per_pair(item_a, item_b, args, device, dtype) -> dict | None:
    """Run independent narrow joint inverses for candidate expert pairs.

    The mixture path above marginalises multiple routed experts inside one
    objective, but its search box is the union of all candidates.  That is useful
    for uncertainty diagnostics and harmful when the union spans a yield-stress
    ridge.  This path keeps the routing uncertainty as a discrete hypothesis:
    each pair receives its own tight box, CMA-ES is run independently, and the
    best joint evidence is selected afterwards.
    """
    members_a = list(item_a["members"])
    members_b = list(item_b["members"])
    max_pairs = int(getattr(args, "shape_joint_max_pairs", 0) or 0)
    candidates: list[tuple[int, int, SubMember, SubMember]] = []
    for ia, ma in enumerate(members_a):
        for ib, mb in enumerate(members_b):
            candidates.append((ia, ib, ma, mb))
    if max_pairs > 0:
        candidates = candidates[:max_pairs]

    child_args = copy.copy(args)
    child_args.shape_joint_pair_mode = "mixture"
    # CI is only meaningful for the final selected pair and is expensive while
    # scanning candidates.  The caller can rerun the selected pair with CI later.
    child_args.shape_report_ci = False

    best: dict | None = None
    pair_rows = []
    total_evals = 0
    t0 = time.time()
    for ia, ib, ma, mb in candidates:
        sub_item_a = _with_single_member(item_a, ma)
        sub_item_b = _with_single_member(item_b, mb)
        try:
            res = inverse_joint_setup(sub_item_a, sub_item_b, child_args, device, dtype)
        except Exception as exc:
            pair_rows.append({
                "ia": ia, "ib": ib,
                "sub_a": int(ma.sub_id), "sub_b": int(mb.sub_id),
                "error": str(exc),
            })
            continue
        if res is None:
            continue
        total_evals += int(res.get("n_rows", 0))
        row = {
            "ia": ia, "ib": ib,
            "sub_a": int(ma.sub_id), "sub_b": int(mb.sub_id),
            "objective": float(res["objective"]),
            "theta_n": float(res["theta_n"]),
            "theta_eta": float(res["theta_eta"]),
            "theta_sy": float(res["theta_sy"]),
            "raw_wrms_total": float(res.get("raw_wrms_total", np.nan)),
            "support_total": float(res.get("support_total", np.nan)),
        }
        pair_rows.append(row)
        if best is None or float(res["objective"]) < float(best["objective"]):
            best = dict(res)
            best["_pair_choice"] = row

    if best is None:
        return None
    best["start_kind"] = "shape_cma_double_per_pair"
    best["n_pairs"] = len(candidates)
    best["n_rows"] = total_evals
    best["opt_s"] = time.time() - t0
    best["pair_candidates"] = pair_rows
    best["pair_choice"] = best.pop("_pair_choice")
    return best
...
def add_argparse_args(ap) -> None:
    ...
    Dead paths (Method A/B priors, sigma_y two-stage prior,
    ...
    ap.add_argument("--shape-cma-space", type=str, default="z",
                    choices=["z", "unit_logit"],
                    help="CMA-ES search coordinates: 'z' optimizes directly in "
                         "(n, log eta, log sigma_y) with bounds; 'unit_logit' "
                         "optimizes unconstrained logit coordinates mapped into "
                         "the same z box.")
    ap.add_argument("--shape-unit-sigma0", type=float, default=1.0,
                    help="CMA-ES initial step when --shape-cma-space=unit_logit")
    ...
    ap.add_argument("--shape-cma-restarts", type=int, default=5,
                    help="Number of CMA-ES multi-starts (different seeds, take best). "
                         "n>1 also reports z_dispersion = std(theta_k) across restarts. "
                         "Large dispersion = multiple basins / ridge degeneracy. "
                         "Default 5 = paper-grade production; use 1 for smoke tests.")
    ...
    # ---- routing ----
    ap.add_argument("--shape-routing-profile", type=str, default="real_robust",
                    choices=["real_robust", "sim_precision", "joint_consistency"],
                    help="Observation-evidence routing profile. "
                         "'real_robust' uses terminal y8 quantile evidence "
                         "for real-video sim-real-gap robustness. "
                         "'sim_precision' uses full-trajectory nearest-neighbor "
                         "evidence for simulator-matched validation. "
                         "Passing --shape-weight-mode overrides the profile.")
    ...
    ap.add_argument("--shape-weight-mode", type=str, default=None,
                    choices=["pca", "gp_fit", "y8_quantile", "chi2", "unified", "full_nn", "joint_consistency"],
                    help="Sub-level routing re-rank: "
                         "'y8_quantile' (production) = pick sub for which "
                         "y_obs[7] is most central in training Y_8 distribution "
                         "(Bayes-evidence-like); "
                         "'full_nn' (sim precision) = nearest full 8-frame "
                         "training trajectory; "
    ...
    ap.add_argument("--shape-gpfit-oversample", type=int, default=3,
                    help="When --shape-weight-mode in {gp_fit, y8_quantile, "
                         "chi2, unified, full_nn}, "
                         "oversample factor for the PCA pre-filter: routing "
                         "fetches K*factor candidates, then re-ranks.")
    ap.add_argument("--shape-noise-floor-cm", type=float, default=0.0,
                    help="Observation-space noise floor for chi2/unified routing modes.")
    ap.add_argument("--shape-joint-pair-mode", type=str, default="mixture",
                    choices=["mixture", "per_pair"],
                    help="How joint inverse handles multiple routed experts. "
                         "'mixture' marginalises them inside one objective and uses "
                         "the union search box. 'per_pair' runs one tight-box joint "
                         "inverse per candidate expert pair and keeps the best joint "
                         "evidence, avoiding union-box ridge wandering.")
    ap.add_argument("--shape-joint-max-pairs", type=int, default=0,
                    help="Optional cap on per-pair joint hypotheses. 0 = all "
                         "pairs from the routed member lists.")
    # ---- loss ----
    ap.add_argument("--shape-gp-noise-log-floor", type=float, default=0.20,
                    help="GP-aware likelihood: log-space measurement noise "
                         ...
                         "materials. Use 0.05 with --no-shape-disable-calib "
                         "only to reproduce pre-2026-05-07 results.")
    ...
    ap.add_argument("--shape-disable-calib",
                    ...
                    help="Skip post-hoc GP variance calibration (treat "
                         ...
                         "(recommended with --shape-gp-noise-log-floor ≥ 0.20; "
                         "0.05 is backward-compat only).")
    ...
    # ---- UQ output ----
    ap.add_argument("--shape-report-ci",
                    action=_argparse.BooleanOptionalAction, default=True,
